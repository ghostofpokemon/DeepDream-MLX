#!/usr/bin/env python3
"""Shared DeepDream parameter presets and helpers."""
...
DEFAULT_INPUT = "input/love.jpg"
if not Path(DEFAULT_INPUT).exists():
    alt_path = Path("love.jpg")
    if alt_path.exists():
        DEFAULT_INPUT = str(alt_path)

MODEL_CONFIG: Dict[str, Dict[str, Any]] = {
    "inception_v3": {
        "weights": "inception_v3_mlx.npz",
        "hero": {
...
def uniform_range(lo: float, hi: float) -> float:
    return random.uniform(lo, hi)


def random_params(
    layer_pool: Sequence[str],
    base_width: int,
    classic: bool,
    classic_layers: Sequence[Sequence[str]] | None = None,
    classic_ranges: Dict[str, Any] | None = None,
    min_dim: float = 120.0,
) -> Dict[str, Any]:
    for _ in range(100):
...
def apply_overrides(params: Dict[str, Any], overrides: Any) -> Dict[str, Any]:
    updated = dict(params)
    if getattr(overrides, "layers", None):
        updated["layers"] = overrides.layers
    for field in ("steps", "lr", "octaves", "scale", "jitter", "smoothing"):
